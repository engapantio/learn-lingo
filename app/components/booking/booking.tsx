...
const BookingForm = ({ teacher, onClose }: BookingFormProps) => {
  const schema = yup.object({
    reason: yup
      .string()
      .oneOf(['career', 'kids', 'abroad', 'exams', 'culture'], 'Select a reason for learning')
      .required(),
  ...
  const onSubmit = async (_data: BookingForm) => {
    ...
    toast.success(
      `Your trial lesson with ${teacher.name} ${teacher.surname} has been booked! We'll contact you shortly.`
    );
    reset();
    onClose();
  };
      ...
      <fieldset className="font-medium mb-5 outline-none border-none">
        ...
        <div className="flex flex-col gap-4">
          {[
            ...
            <label
              key={value}
              className="flex items-center gap-2 cursor-pointer hover:bg-primary-green/5"
            >
              <input
                type="radio"
                value={value}
              ...
              <span
                className={`
      w-5 h-5 rounded-[10px] shrink-0
      border-2 
      flex items-center justify-center
      ${selectedReason === value ? 'border-bg-button' : 'border-bg-dark/20'}
    `}
              >
                <span
                  className={`
        w-2.5 h-2.5 rounded-full
         ${selectedReason === value ? 'bg-bg-button' : 'bg-transparent'}
      `}
                ></span>
              </span>
