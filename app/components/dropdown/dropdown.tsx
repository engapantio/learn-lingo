...
const Dropdown = ({width, label, value, placeholder, options,onChange}: DropdownProps) => {
  ...
  return (
    <fieldset className="relative flex flex-col gap-2" ref={ref}>
      {label && (
        <label className={`min-w-${width} block leading-[1.29] text-sm font-medium text-text-grey`}>
          {label}
        </label>
      )}
      <button
      ...
      {open && (
        <div className={`absolute z-20 mt-1 min-w-${width} bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-auto`}>
          <button
            type="button"
            onClick={() => {
              onChange("");
